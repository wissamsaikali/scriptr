...
function _getStateFromReferer(request) {
  ...
  if (referer) {
    ...
    var redirectUriObj = _split(referer); 
    if (redirectUriObj && redirectUriObj.redirect_uri) {

      var decodedRedirectUri = decodeURIComponent(redirectUriObj.redirect_uri);       
      var redirectUriParams = _split(decodedRedirectUri);
      state = redirectUriParams ? redirectUriParams.state : ""; 
    }
  }
