...
var util = require("ihealth/oauth2/util");
...
function getPersistedTokens(username, tokenType) {
  ...
  var userId = storage.global[config.app + "_" + username + "_userId"];
  if (!accessToken) {
...
/**
 * Return the userid that is associated to the given username
 * @method getUserId
 * @param {String} username
 * @return {String} userId 
 * @throws {Error} if no userId found or if userame is null
 */
function getUserId(username) {
  ...
  if (!username) {
    ...
    throw {
      ...
      "errorDetail": "username cannot be null or empty"
    };
  }
  ...
  if (!userId) {
    ...
    throw {
      ...
      "errorDetail": "Coudl not find a userId to the provided username: " + username
    };    
  }
...
function saveTokens(dto) {
  ...
  console.log("Refesh Token " + dto.refresh_token);
  if (dto.refresh_token) {
  	storage.global[config.app + "_" + username + "_refreshToken"] = dto.refresh_token;
    console.log("Instorage " + storage.global[config.app + "_" + username + "_refreshToken"])
  }
  
  if (dto.userId) {
  	storage.global[config.app + "_" + username + "_userId"] = dto.userId;
    console.log("Instorage " + storage.global[config.app + "_" + username + "_userId"])
  }
  ...
  if (response.status == "200") {
    ...
    if (response.headers["Content-Type"].toLowerCase().indexOf("application/json") > -1) {
      ...
      var dto = {
        ...
        userId: responseObj[config.userIdFieldName],
      };
